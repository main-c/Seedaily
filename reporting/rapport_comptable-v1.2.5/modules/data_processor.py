...
def prepare_suivi_activite_detaille(df: pd.DataFrame, client_code: Optional[str] = None) -> Dict:
    ...
    NOUVEAU FORMAT avec détail par numéro de compte pour chaque catégorie
    ...
    """
    if client_code:
        logger.info(f"Préparation du suivi budgétaire DÉTAILLÉ pour le client: {client_code}")
    else:
        logger.info("Préparation du suivi budgétaire DÉTAILLÉ (mapping par défaut)")
    ...
    for categorie, prefixes in categorie_mapping.items():
        ...
        # Trouver tous les comptes qui correspondent aux préfixes
        comptes_de_la_categorie = set()

        for prefix in prefixes:
            prefix_num = int(prefix.ljust(8, '0'))
            prefix_next = int(str(int(prefix) + 1).ljust(8, '0'))

            comptes_matches = df_copy[
                (df_copy['compte'] >= prefix_num) &
                (df_copy['compte'] < prefix_next)
            ]['compte'].unique()

            comptes_de_la_categorie.update(comptes_matches)

        # Pour chaque compte de la catégorie
        for compte in sorted(comptes_de_la_categorie):
            df_compte = df_copy[df_copy['compte'] == compte]

            # Récupérer le libellé du compte
            libelle = df_compte['libelle'].iloc[0] if len(df_compte) > 0 else ''

            result[categorie]['comptes'][compte] = {
                'libelle': libelle,
                'mois_data': {}
            }
            ...
            for mois_num in range(1, 13):
                ...
                df_compte_mois = df_compte[df_compte['mois_num'] == mois_num]

                # Calculer le montant selon le type (charge ou produit)
                if categorie.startswith('CA_') or categorie == 'Autres_produits':
                    montant = df_compte_mois['credit'].sum()
                else:
                    montant = df_compte_mois['debit'].sum()

                result[categorie]['comptes'][compte]['mois_data'][mois_nom] = montant
        ...
        for mois_num in range(1, 13):
            ...
            for compte_data in result[categorie]['comptes'].values():
                total += compte_data['mois_data'].get(mois_nom, 0)

            # Pour les charges, inverser le signe pour affichage négatif
            if not (categorie.startswith('CA_') or categorie == 'Autres_produits'):
                total = -total

            result[categorie]['total'][mois_nom] = total

    logger.info(f"Suivi budgétaire détaillé préparé: {len(result)} catégories")
    return result
